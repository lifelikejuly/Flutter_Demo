...
class ScaleBoundaries {
  ...
  double get minScale {
    ...
    if (_minScale == PhotoViewComputedScale.fillWidth) {
      return _scaleForFillWidth(outerSize, childSize) *
          (_minScale as PhotoViewComputedScale).multiplier; // ignore: avoid_as
    }
    if (_minScale == PhotoViewComputedScale.fillHeight) {
      return _scaleForFillHeight(outerSize, childSize) *
          (_minScale as PhotoViewComputedScale).multiplier; // ignore: avoid_as
    }
  ...
  double get maxScale {
    ...
    if (_maxScale == PhotoViewComputedScale.fillWidth) {
      return (_scaleForFillWidth(outerSize, childSize) *
          (_maxScale as PhotoViewComputedScale) // ignore: avoid_as
              .multiplier)
          .clamp(minScale, double.infinity);
    }
    if (_maxScale == PhotoViewComputedScale.fillHeight) {
      return (_scaleForFillHeight(outerSize, childSize) *
          (_maxScale as PhotoViewComputedScale) // ignore: avoid_as
              .multiplier)
          .clamp(minScale, double.infinity);
  ...
  double get initialScale {
    ...
    if (_initialScale == PhotoViewComputedScale.fillWidth) {
      return _scaleForFillWidth(outerSize, childSize) *
          (_initialScale as PhotoViewComputedScale) // ignore: avoid_as
              .multiplier;
    }
    if (_initialScale == PhotoViewComputedScale.fillHeight) {
      return _scaleForFillHeight(outerSize, childSize) *
          (_initialScale as PhotoViewComputedScale) // ignore: avoid_as
              .multiplier;
    }
...
double _scaleForFillWidth(Size size, Size childSize) {
  final double imageWidth = childSize.width;
  final double imageHeight = childSize.height;
...
double _scaleForFillHeight(Size size, Size childSize) {
  final double imageWidth = childSize.width;
  final double imageHeight = childSize.height;
