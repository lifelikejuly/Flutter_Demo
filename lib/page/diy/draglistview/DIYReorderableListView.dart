...
/// The callback used by [DIYReorderableListView] to move an item to a new
/// position in a list.
///
...
/// [DIYReorderableListView] will need to account for this when inserting before
/// [newIndex].
///
...
///
/// {@youtube 560 315 https://www.youtube.com/watch?v=3fB1mxOsqJE}
class DIYReorderableListView extends StatefulWidget {

  /// Creates a reorderable list.
  DIYReorderableListView({
    Key key,
    this.header,
    this.footer,
    this.extraWrapper,
  ...
  final List<Widget> extraWrapper;
  ...
  /// underlying list.
  ///
  /// This [DIYReorderableListView] calls [onReorder] after a list child is dropped
  /// into a new position.
  final ReorderCallback onReorder;

  @override
  _DIYReorderableListViewState createState() => _DIYReorderableListViewState();
}
...
class _DIYReorderableListViewState extends State<DIYReorderableListView> {
  // We use an inner overlay so that the dragging list item doesn't draw outside of the list itself.
  final GlobalKey _overlayKey = GlobalKey(debugLabel: '$DIYReorderableListView overlay key');
    ...
    _listOverlayEntry = OverlayEntry(
      ...
      builder: (BuildContext context) {
        return _ReorderableListContent(
          ...
          extraWrapper: widget.extraWrapper,
...
class _ReorderableListContentState extends State<_ReorderableListContent> with TickerProviderStateMixin<_ReorderableListContent> {
  ...
  Widget _wrap(Widget toWrap, int index, BoxConstraints constraints) {
    ...
    Widget extraCell;
    if(widget.extraWrapper != null && widget.extraWrapper.length > index){
      extraCell = widget.extraWrapper[index];
    }
    ...
    Widget buildDragTarget(BuildContext context, List<Key> acceptedCandidates, List<dynamic> rejectedCandidates) {
      ...
      Widget child = LongPressDraggable<Key>(
        ...
        child: _dragging == toWrap.key ? const SizedBox() :
        ((extraCell != null && _dragging == null) ? Stack(children: <Widget>[toWrapWithSemantics,extraCell],) : toWrapWithSemantics),
