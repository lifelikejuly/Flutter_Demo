...
class DIYWrap extends MultiChildRenderObjectWidget {
...
class RenderWrap extends RenderBox with ContainerRenderObjectMixin<RenderBox, WrapParentData>,
  ...
  double _computeIntrinsicHeightForWidth(double width) {
    print("<> _computeIntrinsicHeightForWidth $width");
  ...
  void performLayout() {
    ...
    while (child != null && line < 2) {
      ...
      print("<> performLayout mainAxisExtent $mainAxisExtent runMainAxisExtent $runMainAxisExtent mainAxisLimit $mainAxisLimit constraints ${constraints.maxHeight}");
    }
    if (childCount > 0) {
      mainAxisExtent = math.max(mainAxisExtent, runMainAxisExtent);
      crossAxisExtent += runCrossAxisExtent;
    ...
    print("<> performLayout runCount $runCount");
    assert(runCount > 0);
    ...
    child = firstChild;
    for (int i = 0; i < 1; ++i) {
      print("<> performLayout child runCount $i");
  ...
  void paint(PaintingContext context, Offset offset) {
    print("<> paint offset ${offset.toString()}");
