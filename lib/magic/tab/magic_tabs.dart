...
class MagicTab extends StatelessWidget implements PreferredSizeWidget {
...
///https://juejin.cn/post/6844903746720251911#heading-0
///tab字体抖动问题解决
class _TabStyle extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Animation<double> colorAnimation = _ChangeAnimation(tabController);
...
class _IndicatorPainter extends CustomPainter {
  ...
  Rect indicatorRect(Size MagicTabBarSize, int tabIndex) {
    assert(_currentTabOffsets != null);
    ...
    final Rect rect = Rect.fromLTWH(tabLeft, 0.0, tabRight - tabLeft, MagicTabBarSize.height);
...
class _MagicTabBarScrollPosition extends ScrollPositionWithSingleContext {
  _MagicTabBarScrollPosition({
    @required ScrollPhysics physics,
    @required ScrollContext context,
    @required ScrollPosition oldPosition,
    @required this.magicTabBar,
  }) : super(
    physics: physics,
    context: context,
    initialPixels: null,
  ...
  final _MagicTabBarState magicTabBar;
  ...
  bool applyContentDimensions(double minScrollExtent, double maxScrollExtent) {
    ...
    if (_initialViewportDimensionWasZero != true) {
      ...
      correctPixels(magicTabBar._initialScrollOffset(viewportDimension, minScrollExtent, maxScrollExtent));
      result = false;
    }
...
class _MagicTabBarScrollController extends ScrollController {
  _MagicTabBarScrollController(this.magicTabBar);

  final _MagicTabBarState magicTabBar;

  @override
  ScrollPosition createScrollPosition(ScrollPhysics physics, ScrollContext context, ScrollPosition oldPosition) {
    return _MagicTabBarScrollPosition(
      physics: physics,
      context: context,
      oldPosition: oldPosition,
      magicTabBar: magicTabBar,
    );
  }
}
...
class MagicTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  @override
  State<MagicTabBar> createState() => _MagicTabBarState();
}

class _MagicTabBarState extends State<MagicTabBar> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_controller != null) {
      ...
      print("<> _currentIndex $_currentIndex previousIndex $previousIndex");
      if (_controller.indexIsChanging) {
        // The user tapped on a tab, the tab controller's animation is running.
        assert(_currentIndex != previousIndex);
        print("<> indexIsChanging _currentIndex $_currentIndex previousIndex $previousIndex");
        final Animation<double> animation = _ChangeAnimation(_controller);
        wrappedTabs[_currentIndex] = _buildStyledTab(wrappedTabs[_currentIndex],_currentIndex,_currentIndex,true, animation,_controller);
        wrappedTabs[previousIndex] = _buildStyledTab(wrappedTabs[previousIndex],previousIndex,_currentIndex, false, animation,_controller);
      } else {
        // The user is dragging the TabBarView's PageView left or right.
        final int tabIndex = _currentIndex;
        final Animation<double> centerAnimation = _DragAnimation(_controller, tabIndex);
        wrappedTabs[tabIndex] = _buildStyledTab(wrappedTabs[tabIndex],tabIndex, _currentIndex, true, centerAnimation,_controller);
        if (_currentIndex > 0) {
          final int tabIndex = _currentIndex - 1;
          final Animation<double> previousAnimation = ReverseAnimation(_DragAnimation(_controller, tabIndex));
          wrappedTabs[tabIndex] = _buildStyledTab(wrappedTabs[tabIndex],tabIndex, _currentIndex, false, previousAnimation,_controller);
        }
        if (_currentIndex < widget.tabs.length - 1) {
          final int tabIndex = _currentIndex + 1;
          final Animation<double> nextAnimation = ReverseAnimation(_DragAnimation(_controller, tabIndex));
          wrappedTabs[tabIndex] = _buildStyledTab(wrappedTabs[tabIndex],tabIndex, _currentIndex, false, nextAnimation,_controller);
        }
      }
    }
    ...
    if (widget.isScrollable) {
      _scrollController ??= _MagicTabBarScrollController(this);
      magicTabBar = SingleChildScrollView(
...
class MagicTabBarView extends StatefulWidget {
  ...
  @override
  State<MagicTabBarView> createState() => _MagicTabBarViewState();
}

class _MagicTabBarViewState extends State<MagicTabBarView> {
  TabController _controller;
