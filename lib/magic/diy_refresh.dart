...
class _CupertinoSliverRefresh extends SingleChildRenderObjectWidget {
  ...
  @override
  RenderCupertinoSliverRefresh createRenderObject(BuildContext context) {
    return RenderCupertinoSliverRefresh(
  ...
  @override
  void updateRenderObject(BuildContext context, covariant RenderCupertinoSliverRefresh renderObject) {
    renderObject
...
class RenderCupertinoSliverRefresh extends RenderSliver
    with RenderObjectWithChildMixin<RenderBox> {
  RenderCupertinoSliverRefresh({
  ...
  void performLayout() {
    ...
    //<DIY> 取消了layoutExtent的高度
    // The new layout extent this sliver should now have.
    final double layoutExtent =
        (_hasLayoutExtent ? 1.0 : 0.0) * _refreshIndicatorExtent;
//    final double layoutExtent = 0;
    // If the new layoutExtent instructive changed, the SliverGeometry's
    // layoutExtent will take that value (on the next performLayout run). Shift
    // the scroll offset first so it doesn't make the scroll position suddenly jump.
    if (layoutExtent != layoutExtentOffsetCompensation) {
//      print("<AnimatedBuilder> layoutExtent  $layoutExtent layoutExtent != layoutExtentOffsetCompensation $layoutExtentOffsetCompensation");
...
/// the same values passed into the [DIYCupertinoSliverRefreshControl].
///
...
/// A callback function that's invoked when the [DIYCupertinoSliverRefreshControl] is
/// pulled a `refreshTriggerPullDistance`. Must return a [Future]. Upon
/// completion of the [Future], the [DIYCupertinoSliverRefreshControl] enters the
...
///    [DIYCupertinoSliverRefreshControl] is part of the scrollable and actively occupies
///    scrollable space.
class DIYCupertinoSliverRefreshControl extends StatefulWidget {
  ...
  /// a refresh.
  const DIYCupertinoSliverRefreshControl({
    Key key,
  ...
  /// Retrieve the current state of the DIYCupertinoSliverRefreshControl. The same as the
  /// state that gets passed into the [builder] function. Used for testing.
  @visibleForTesting
  static RefreshIndicatorMode state(BuildContext context) {
    final DIYCupertinoSliverRefreshControlState state = context.findAncestorStateOfType<DIYCupertinoSliverRefreshControlState>();
...
//            : CupertinoActivityIndicator(radius: 14.0),
  ...
  @override
  DIYCupertinoSliverRefreshControlState createState() => DIYCupertinoSliverRefreshControlState();
}

class DIYCupertinoSliverRefreshControlState extends State<DIYCupertinoSliverRefreshControl> {
      ...
      case RefreshIndicatorMode.drag:
        if (latestIndicatorBoxExtent == 0) {
          ...
          if (widget.onRefresh != null) {
            print("<AnimatedBuilder> SliverRefresh _onRefresh ");
